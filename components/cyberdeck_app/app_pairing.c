...
static ble_device_info_t s_devs[PAIR_MAX];
static int      s_ndevs;
static int      s_sel;
static uint64_t s_last_poll;
static uint64_t s_last_activity;
static bool     s_forget_armed;     /* "Forget bonds" needs a 2nd tap */
...
static void render_pairing(uint64_t now)
{
    ...
    ui_pen(s_ndevs ? OVERLAY_COL_GREEN : OVERLAY_COL_AMBER);
    ui_putch(2, 1, s_ndevs ? UI_LED_ON : spinner_glyph(app.anim_frame), 0);
    ui_pen(OVERLAY_COL_DEFAULT);
    if (s_ndevs)
        ui_printf(4, 1, 0, "%d found - select your keyboard", s_ndevs);
    else
    ...
    int ndev = s_ndevs > cap - 2 ? cap - 2 : s_ndevs;
    g.count  = ndev + 2;
    app.grid = g;
    if (s_sel >= g.count) s_sel = g.count - 1;
...
static void pairing_select(int slot, uint64_t now)
{
    ...
    if (slot < nd && app.cfg.ble) {                /* a discovered device */
        app.cfg.ble->select_device(s_devs[slot].addr, s_devs[slot].addr_type);
        toast(now, "pairing %.32s...", s_devs[slot].name);
        exit_pairing(now);
...
{
    if (now - s_last_activity > PAIR_TIMEOUT_MS) {
    ...
    if (now - s_last_poll >= PAIR_POLL_MS && app.cfg.ble) {
        s_last_poll = now;
        ble_device_info_t fresh[PAIR_MAX];
        int n = app.cfg.ble->get_scan_results(fresh, PAIR_MAX);
        if (n != s_ndevs ||
            memcmp(fresh, s_devs, (size_t)n * sizeof(fresh[0])) != 0) {
            memcpy(s_devs, fresh, sizeof(fresh));
            s_ndevs = n;
            if (s_sel >= n) s_sel = n ? n - 1 : 0;
            s_last_activity = now;   /* results still arriving */
            render_pairing(now);
        }
    }
}
...
void pairing_input(const cyberdeck_input_t *ev, ui_key_t k, char ch, uint64_t now)
{
    ...
    case K_UP: case K_DOWN: case K_LEFT: case K_RIGHT: {
        ...
        if (ns != s_sel) {
            s_sel = ns;
            s_forget_armed = false;   /* moving away backs down */
            render_pairing(now);
        }
        break;
    }
    case K_ENTER:
        pairing_select(s_sel, now);
        break;
    case K_ESC:
        exit_pairing(now);
        break;
